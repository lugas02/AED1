...
int Palindromo() {
    ...
    for( int i = 0; i < novaPilha->tamanho; i++ ){
...
int main() {
    ...
    while ( letra != 'x' ) {
        scanf(" %c", &letra); // scanf não é seguro
        //letra = getchar();  // Adiciona um '\0' no fim de (quase) cada novo caractere(hipótese: '\0'[enter] fica no buffer e é usado quando é chamado)
